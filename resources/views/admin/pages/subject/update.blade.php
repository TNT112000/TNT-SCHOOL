                    ...
                    <div class="section" id="title-page">
                        <div class="clearfix">
                            <div class="title_update">
                                <h3 id="index" class="fl-left">Chỉnh sửa môn học</h3>
                            ...
                            <form method="POST" action="{{ route('subject.update', ['subject' => $subject->id]) }}">
                                ...
                                <div class="request_input">
                                    ...
                                    <input type="text" name="code" id="title" class="with_input"
                                        value="{{ old('code') ? old('code') : $subject->code }}" readonLy>
                                    @error('code')
                                ...
                                <div class="request_input">
                                    ...
                                    <select class="select_list" name="faculty_id" id="faculty_id">
                                        ...
                                        <option
                                            value="{{ old('faculty_id') ? old('faculty_id') : $subject->faculty_id }}">
                                            {{ old('faculty_id') ? App\Models\Faculty::find(old('faculty_id'))->name : $subject->faculty->name }}
                                        </option>
                                        @foreach ($faculty as $item)
                                            @if (old('faculty_id') ? $item->id != old('faculty_id') : $item->id != $subject->faculty_id)
                                ...
                                <div class="request_input">
                                    ...
                                    <select class="select_list" name="specialized_id" id="specialized_id">
                                        ...
                                        @else
                                            <option
                                                value="{{ old('specialized_id') ? old('specialized_id') : $subject->specialized_id }}">
                                                {{ old('specialized_id') ? App\Models\Specialized::find(old('specialized_id'))->name : $subject->specialized->name }}
                                            </option>
                                        @endif
                                        {{-- @foreach ($specialized as $item)
                                            @if (old('specialized_id') ? $item->id != old('specialized_id') : $item->id != $subject->specialized_id)
                                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                            @endif
                                        @endforeach --}}
                                    </select>
                                ...
                                <div class="display_flex box_btn">
                                    <button type="submit" name="btn-submit" class="btn_submit" id="btn-submit">Cập
                                        nhập</button>
                                    <a href="{{ route('subject.destroy', ['subject' => $subject->id]) }}"
